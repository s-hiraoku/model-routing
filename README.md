...
bun run evals -- run --batch 2026-W28 --stage replay
...
評価 replay は run ごとに専用 gateway をエフェメラルポートへ起動する。`mid+demote` は専用 gateway の passthrough モード上で agent step だけ low tier へ書き換え、`requests.replay_run_id` と `shift_events` に記録するため、本番 gateway の状態やトラフィックには影響しない。
