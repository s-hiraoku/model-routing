...
**M0 コード実装完了・運用ゲート確認中。M1 は分類・サンプラーの実装中。** 実装レベルの設計は [docs/](docs/) にある:
...
M1 の `classify` はデフォルトではヒューリスティックのみで、`--llm` を付けたときだけ低信頼・unknown タスクを Claude Agent SDK で分類する。`sample` は `--yes` なしでは dry run として見積もりだけ表示する。
