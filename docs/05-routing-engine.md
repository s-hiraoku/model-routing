...
overrides:                           # 手動上書き。再生成時もマージ保持
  ...
  docs:
    action: force                    # force = このカテゴリを指定 tier に固定
    to: mid
...
decide(features: RequestFeatures, state: SessionShiftState): ShiftDecision
 ...
 ├─ 2. overrides.<category>.action: force → 指定 tier へ固定(reason: override_force)
 ├─ 3. agent_step 判定(ターン種別。タスク文脈がなくても動く):
...
- `overrides.<category>.action: force` は人間が承認したカテゴリ固定で、policy 生成ルールより優先する
...
export interface ShiftDecision {
  ...
  reason: 'demote_agent_step' | 'demote_task' | 'promote_task' | 'hold'
        | 'hold_sticky' | 'quota_governor' | 'degrade_guard' | 'override_force';
