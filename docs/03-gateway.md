...
gateway は 127.0.0.1 バインドのみ。`/internal/*` と `X-MR-Variant` は localhost 由来でのみ受理し、`X-MR-Variant` は上流へ転送しない。M2 の replayer は run ごとに `createGatewayApp` で固定 `run_id` / `variant` を持つ専用 gateway をエフェメラルポートへ起動し、Agent SDK の `ANTHROPIC_BASE_URL` をそのポートへ向ける。本番 gateway に replay の共有状態や制御 endpoint は持たせない。
...
受信 POST /v1/messages
 ...
 │ 4. variant 解決: 専用 gateway の固定 replay context / X-MR-Variant ヘッダ or 本番ポリシー
