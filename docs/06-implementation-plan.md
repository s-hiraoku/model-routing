...
- [ ] 夜間ジョブ `evals -- nightly`: 暗黙シグナル集計(ESC 中断率・訂正プロンプト率の gear × カテゴリ別)、劣化判定 → ルール自動サスペンド、新モデル検知(未知 model_requested パターン)、日次スナップショット (訂正数・未知モデル・shift 後エラー集計、`--policy` 指定時のカテゴリ自動 suspend は実装済み)
- [ ] 訂正マーカー検出の実装と較正(「違う」「そうじゃなくて」「やり直し」等。誤検出率を目視確認)
- [ ] 自動ロールバック: サスペンド → changelog(origin: auto_rollback)→ 通知 → 復帰は次バッチ再検証 (`nightly --policy` の suspend + changelog は実装済み)
- [x] ドリフト検知: バッチ間で win_rate が CI を超えて動いたカテゴリを警告
- [x] 新モデル対応の実演: models.yaml の tier 差し替え 1 行 → 自動再評価 → ポリシー再生成(07 のシナリオを 1 回実走) (`model-handoff` で次バッチ手順と枠見積もりを生成)

### 人間ループ

- [x] `feedback.yaml`(注意予算・通知・変更統治)+ config ローダー
...
- [ ] フィードバック取り込み: LLM 解釈(parsed_json)→ 変更案生成 → review-ui 提案ページで承認/却下 → 反映 + changelog(origin: human_feedback) (`feedback_proposals` 生成、承認/却下 UI、accepted proposal の policy/changelog 反映は local heuristic で実装済み)
- [ ] 満足度チェック(月 1 問): 通知 → 満足度ページ → 「止めたい」は即 passthrough 化
- [x] `bun run policy -- rollback <version>`(即時巻き戻し)
